...
class ControllerListComponent extends Component {
	...
	public function getControllers() {
		...
		//$controllerClasses[-1]="All";
                $remove_con=array('App','Pages','UserGroupPermissions','UserMgmtApp');
		$plugins = App::objects('plugins');
		foreach ($plugins as $p) {
			$pluginControllerClasses = App::objects($p.'.Controller');
			foreach ($pluginControllerClasses as $controller) {
		...
		ksort($controllerClasses);
                //remove not necessary controllers
                 $controllerClasses=array_diff($controllerClasses,$remove_con);
                 /*
                 $controllerClasses=array_diff($controllerClasses,['Pages']);
                 $controllerClasses=array_diff($controllerClasses,['UserGroupPermissions']);
                 $controllerClasses=array_diff($controllerClasses,['UserMgmtApp']);
                */
