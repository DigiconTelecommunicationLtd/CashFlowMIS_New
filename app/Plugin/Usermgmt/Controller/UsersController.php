...
class UsersController extends UserMgmtAppController {
	...
	public function index() {
		...
		$users=$this->User->find('all', array('order'=>'User.id desc'));
                $this->Session->write('UserList',$users);
				...
				if ($user['User']['password'] === $hashed) {
					...
					if ($remember) {
						$this->UserAuth->persist('2 weeks');
                                        ...
                                        $users=$this->User->find('list', array('order'=>'User.id desc','fields'=>array('id','username')));
                                        $this->Session->write('UserList',$users);
	...
	public function addUser() {
		...
		$userGroups=$this->UserGroup->getGroups($alias_name);
		$this->set('userGroups', $userGroups);
		if ($this->request -> isPost()) {
			$this->User->set($this->data);
			if ($this->User->RegisterValidate()) {
	...
	public function dashboard() {	
		...
		$userId=$this->UserAuth->getUserId();
		$user = $this->User->findById($userId);
		$this->set(compact('user','start_date','start_end','title','data'));
	}
