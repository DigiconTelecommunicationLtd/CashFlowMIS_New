    ...
    <div class="x_content">        
        ...
        <div class="form-group">
            ...
            <div class="col-md-2 col-sm-2 col-xs-12 form-group">
                ...
                <?php echo $this->Form->input("balance_greater_than", array("type" => "text", 'label' => false, 'div' => false, 'class' => "form-control col-md-7 col-xs-12",  'id' => 'balance_greater_than', 'required' => true,'value'=>$balance_greater_than)) ?>
            </div>
            <div class="col-md-2 col-sm-2 col-xs-12 form-group">
                <label>Balance Carry</label>
                <?php echo $this->Form->input("balance_carry", array("type" => "select", 'label' => false, 'div' => false, 'class' => "select2_single form-control col-md-7 col-xs-12", 'tabindex' => -1, 'empty' => '', 'id' => 'balance_carry', 'required' =>true,'options'=>array('no'=>'No','yes'=>'Yes'))) ?>
            </div>
        ...
        <table id="datatable-buttons" class="table table-striped table-bordered">
            ...
            <tbody>
                ...
                foreach ($results as $result): #echo '<pre>';print_r($result);exit;
                 ...
                 /*identify carry information*/
                         $delivery_amount=h($result[0]['delivery_amount']);
                        $invoice_amount=h(round(($delivery_amount*$result['Contract']['billing_percent_progressive'])/100,3)); 
                        $amount_received=($collection_history_collection)? $collection_history_collection[0]['amount_received']:0.00;
                        $ajust_adv_amount=($collection_history_collection)? $collection_history_collection[0]['ajust_adv_amount']:0.00;
                        $ait=($collection_history_collection)? $collection_history_collection[0]['ait']:0.00;
                        $vat=($collection_history_collection)? $collection_history_collection[0]['vat']:0.00;
                        $ld=($collection_history_collection)? $collection_history_collection[0]['ld']:0.00;
                        $other_deduction=($collection_history_collection)? $collection_history_collection[0]['other_deduction']:0.00;                   
                        $balance=round($invoice_amount-($amount_received+$ait+$vat+$ld+$other_deduction),3);
                        #check carray is greater than balance amount
                        
                        if($balance>=$balance_greater_than){
                ?>
                    <tr>                       
